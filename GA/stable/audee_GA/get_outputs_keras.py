...
try:
    ...
    r.hset(f"errors_{predictions_key}", keras_bk, pickle.dumps([e])) # log error if there is
    sys.exit(e)
else:
    er_layers = []
    isnan = False
    for i in range(len(outputs)): # append layers with nan outputs
        if np.isnan(outputs[i]).any():
            isnan = True
            er_layers.append(i)
    
    if isnan: # if there exists nan
        errors.append('nan')
        errors.append(keras_bk)
...
'''
Example usage:
python get_outputs_keras.py tensorflow 1 0 0 0 -2
'''